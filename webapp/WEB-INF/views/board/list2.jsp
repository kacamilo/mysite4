...
<body>
	<div id="wrap">
		...
		<div id="content">
			...
			<div id="board">
				<div id="list">
					<form action="${pageContext.request.contextPath }/board/list2"
						method="get">
							...
							<c:forEach items="${pMap.boardList}" var="boardVo">
								<tr>
									...
									<td class="text-left"><a
										href="${pageContext.request.contextPath }/board/read/${boardVo.no}">${boardVo.title }</a></td>
									<td>${boardVo.userName }</td>
									<td>${boardVo.hit }</td>
									<td>${boardVo.regDate }</td>
									<td><c:if test="${boardVo.userNo == authUser.no }">
											<a
												href="${pageContext.request.contextPath }/board/delete?no=${boardVo.no}">[삭제]</a>
										</c:if></td>
								</tr>
							</c:forEach>
						</tbody>
					...
					<div id="paging">
						...
						<ul>
							<c:if test="${pMap.prev == true }">
								<li><a
									href="${pageContext.request.contextPath }/board/list2?crtPage=${startPageBtnNo-1 }">◀</a></li>
							</c:if>
							...
							<c:if test="${pMap.next == true }">
								<li><a
									href="${pageContext.request.contextPath }/board/list2?crtPage=${endPageBtnNo-1 }">▶</a></li>
							</c:if>
						</ul>
